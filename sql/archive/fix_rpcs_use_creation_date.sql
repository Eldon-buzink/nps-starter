-- Fix RPC functions to use creation_date instead of created_at
-- This will enable proper month-over-month analysis and trends

-- Drop existing functions first
DROP FUNCTION IF EXISTS themes_promoter_detractor(DATE,DATE,TEXT,TEXT);
DROP FUNCTION IF EXISTS themes_aggregate(DATE,DATE,TEXT,TEXT,TEXT);
DROP FUNCTION IF EXISTS v_nps_summary(DATE,DATE,TEXT,TEXT);
DROP FUNCTION IF EXISTS nps_trend_overall(DATE,DATE,TEXT,TEXT);
DROP VIEW IF EXISTS v_nps_response_themes;

-- Monthly overall trends (using creation_date)
...
LANGUAGE SQL STABLE AS $$
  WITH base AS (
    SELECT DATE_TRUNC('month', creation_date)::DATE m, nps_score
    FROM nps_response
    WHERE (p_start IS NULL OR creation_date >= p_start)
      AND (p_end   IS NULL OR creation_date <  p_end + INTERVAL '1 day')
...
-- Summary for KPI cards with MoM (using creation_date)
...
LANGUAGE SQL STABLE AS $$
  ...
  prev_window AS (
    SELECT
      MIN(DATE_TRUNC('month', creation_date)) AS minm
    FROM base
  ),
  prev AS (
    SELECT
    ...
    FROM nps_response
    WHERE creation_date >= (SELECT minm - INTERVAL '1 month' FROM prev_window)
      AND creation_date <  (SELECT minm FROM prev_window)
      AND (p_survey IS NULL OR survey_name = p_survey)
      AND (p_title  IS NULL OR title_text = p_title)
  )
  SELECT
...
-- Themes exploded view (using creation_date)
...
SELECT response_id, creation_date, survey_name, title_text, nps_score, sentiment_score, unnest(themes) AS theme
FROM exploded;

-- Per-theme aggregates (using creation_date)
...
LANGUAGE SQL STABLE AS $$
  WITH base AS (
    ...
    WHERE (p_start_date IS NULL OR t.creation_date >= p_start_date)
      AND (p_end_date   IS NULL OR t.creation_date <  p_end_date + INTERVAL '1 day')
...
-- Promoters vs Detractors per theme (using creation_date)
