...
export async function getMomMoves(
  startDate?: string,
  endDate?: string,
  ...
  try {
    ...
    // Get title data for current and last month
    ...
    if (currentError || lastError) {
      console.error('Error fetching mom moves data:', currentError || lastError);
      return [];
    }
    
    // Calculate NPS for each title in both months
    ...
    const currentNPS = calculateNPS(currentData || []);
    const lastNPS = calculateNPS(lastData || []);
    
    // Calculate month-over-month changes
    const changes: MomMove[] = [];
    ...
    currentNPS.forEach((currentScore, title) => {
      ...
      if (Math.abs(change) > 0.1) { // Only include significant changes
        changes.push({
          title,
          current_nps: currentScore,
          previous_nps: lastScore,
          mom_delta: change,
          current_responses: currentData?.filter(r => r.title_text === title).length || 0,
          previous_responses: lastData?.filter(r => r.title_text === title).length || 0
        });
      }
    });
    
    // Sort by absolute change and return top K
    return changes
      .sort((a, b) => Math.abs(b.mom_delta) - Math.abs(a.mom_delta))
      .slice(0, topK);
      
  } catch (error) {
    console.error('Error in getMomMoves:', error);
    return [];
  }
}
